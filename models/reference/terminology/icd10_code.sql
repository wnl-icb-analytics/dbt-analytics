with definitions as (
    ...
    from {{ ref('stg_reference_icd10_code_term_latest') }}

    union all

    select replace(upper(trim(code)), '.', ''), description,
        'Dictionary.dbo.Diagnosis', 2, date_updated::timestamp_ntz, sk_diagnosis_id
    from {{ ref('stg_dictionary_dbo_diagnosis') }}
)
