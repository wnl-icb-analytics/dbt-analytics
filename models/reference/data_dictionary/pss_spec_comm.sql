with cs as (
    select
        split_part(npo_c_category, ' - ', 1)  as spec_comm_code,
        split_part(npo_c_category, ' - ', 2) as spec_comm_desc,
        case
    ...
    where ps_flag not like '%_Exc%'

    qualify 
        file_period = max(file_period) over (partition by spec_comm_code)
        and row_number() over (partition by spec_comm_code order by spec_comm_desc) = 1
)
