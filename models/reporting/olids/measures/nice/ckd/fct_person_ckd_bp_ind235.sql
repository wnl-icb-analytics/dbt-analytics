...
assessed AS (
    SELECT
        ...
        COALESCE(bp.latest_bp_date::DATE BETWEEN DATEADD(month, -12, CURRENT_DATE()) AND CURRENT_DATE(), FALSE) AS is_bp_recorded_in_last_12m,
        CASE
...
status AS (
    SELECT
        *,
        COALESCE(latest_systolic_value < indicator_systolic_threshold
...
SELECT
    person_id,
    'IND235' AS indicator_id,
    'Kidney conditions: CKD and blood pressure when ACR less than 70' AS indicator_name,
