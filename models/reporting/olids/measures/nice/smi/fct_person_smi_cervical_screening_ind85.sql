...
WITH indicator_population AS (
    ...
    WHERE profile.has_active_smi_diagnosis AND demographics.gender = 'Female' AND age.age BETWEEN 25 AND 64
),
...
assessed AS (
    SELECT
        ...
        CASE WHEN population.latest_completed_date >= DATEADD(month, -60, CURRENT_DATE()) THEN population.latest_completed_date END AS latest_record_date,
        COALESCE(population.latest_completed_date >= DATEADD(month, -60, CURRENT_DATE()), FALSE) AS is_in_numerator
