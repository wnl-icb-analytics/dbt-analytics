...
assessed AS (
    SELECT
        ...
        CASE WHEN population.latest_blood_pressure_date >= DATEADD(month, -15, CURRENT_DATE()) THEN population.latest_blood_pressure_date END AS latest_record_date,
        COALESCE(population.latest_blood_pressure_date >= DATEADD(month, -15, CURRENT_DATE()), FALSE) AS is_in_numerator
