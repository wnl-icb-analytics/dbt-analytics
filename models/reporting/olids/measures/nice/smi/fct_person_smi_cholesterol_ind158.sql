...
WITH indicator_population AS (
    ...
    WHERE profile.has_active_smi_diagnosis AND age.age >= 18
        AND NOT COALESCE(profile.earliest_cvd_diagnosis_date < DATEADD(month, -12, CURRENT_DATE()), FALSE)
),
...
assessed AS (
    SELECT
        ...
        CASE WHEN population.latest_cholesterol_hdl_ratio_date >= DATEADD(month, -12, CURRENT_DATE()) THEN population.latest_cholesterol_hdl_ratio_date END AS latest_record_date,
        COALESCE(population.latest_cholesterol_hdl_ratio_date >= DATEADD(month, -12, CURRENT_DATE()), FALSE) AS is_in_numerator
