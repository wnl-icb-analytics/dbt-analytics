...
-- Learning disability health check and health action plan both recorded in 12 months for people aged 14 and over on the learning disability register.
...
assessed AS (
    SELECT
        ...
        COALESCE(population.latest_ld_health_check_date >= DATEADD(month, -12, CURRENT_DATE()), FALSE)
            AND COALESCE(population.latest_ld_health_action_plan_date >= DATEADD(month, -12, CURRENT_DATE()), FALSE) AS is_in_numerator
