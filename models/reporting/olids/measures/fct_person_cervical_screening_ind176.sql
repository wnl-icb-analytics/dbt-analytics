...
assessed AS (
    SELECT
        ...
        CASE WHEN screening.latest_completed_date >= DATEADD(month, -42, CURRENT_DATE())
            THEN screening.latest_completed_date END AS latest_record_date,
        COALESCE(screening.latest_completed_date >= DATEADD(month, -42, CURRENT_DATE()), FALSE) AS is_in_numerator
