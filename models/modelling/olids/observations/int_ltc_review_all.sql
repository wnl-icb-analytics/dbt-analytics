...
/*
Long-term condition review, care plan and health check records from the PCD
review clusters, one row per observation and cluster. review_type names the
record from its cluster: asthma, COPD, heart failure, rheumatoid arthritis and
...
FROM ({{ get_observations("'REV_COD', 'COPDRVW_COD', 'HFRVW_COD', 'HFMEDRVW_COD', 'MEDRVW_COD', 'RARTHRVW_COD', 'HLTHCHK_COD', 'HLTHAP_COD', 'MDRV_COD', 'DEMCP_COD', 'DEMCPRVW_COD', 'DEPRVW_COD'", source='PCD') }}) obs
