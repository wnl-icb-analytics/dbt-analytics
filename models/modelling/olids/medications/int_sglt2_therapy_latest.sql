...
Consumers apply their own window to latest_order_date; the NICE indicators use
six calendar months before the reporting date. Orders before 1990 or after the
build date are ignored.
*/

WITH orders AS (
    SELECT person_id, medication_order_id, order_date, sglt2_drug, bnf_name
    FROM {{ ref('int_sglt2_medications_all') }}
    WHERE order_date BETWEEN '1990-01-01' AND CURRENT_DATE()
),

per_person AS (
    SELECT person_id, MIN(order_date) AS first_order_date, COUNT(*) AS order_count
    FROM orders
    GROUP BY person_id
),
...
SELECT
    ...
    latest.sglt2_drug AS latest_sglt2_drug,
