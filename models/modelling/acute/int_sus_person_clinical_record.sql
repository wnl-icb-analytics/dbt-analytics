...
with procedure_labels as (
    select replace(upper(trim(code)), '.', '') as code, description
    from {{ ref('stg_dictionary_dbo_procedure') }}
    qualify row_number() over (
        partition by replace(upper(trim(code)), '.', '')
        order by date_updated desc nulls last, sk_procedure_code desc
    ) = 1
), clinical_records as (
select
...
left join {{ ref('stg_dictionary_dbo_diagnosis') }} as label on s.code = label.code
where true
...
left join {{ ref('obt_encounter_apc') }} as p on s.primarykey_id = p.visit_occurrence_id
left join {{ ref('stg_sus_apc_spell') }} as delivery on s.primarykey_id = delivery.primarykey_id
left join procedure_labels as label on replace(upper(trim(s.code)), '.', '') = label.code
where true
