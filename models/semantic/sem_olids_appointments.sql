...
DIMENSIONS(
    ...
    -- Contact mode
    appt.contact_mode AS contact_mode WITH SYNONYMS = ('mode', 'delivery mode') COMMENT = 'Simplified contact mode: Face-to-face, Telephone, Online, Home Visit, or Unknown. Video appears only on about 4.5k records from 2021-2023; later video consultations are not recorded as a separate mode.',
    ...
    demographics.imd_quintile_25 AS imd_quintile_25 COMMENT = 'Current-residence IMD 2025 quintile text label: ''Most Deprived'', ''Second Most Deprived'', ''Third Most Deprived'', ''Second Least Deprived'', ''Least Deprived'', or ''Unknown''',
...
METRICS(
    -- Volume
    appt.appointment_count AS COUNT(appt.appointment_id) COMMENT = 'All retained clinical appointment records. Every record is attended or DNA: OLIDS carries no cancelled status, and available and future slots are excluded upstream.',
    ...
    -- DNA rate
    appt.dna_rate AS COUNT(CASE WHEN appt.is_dna THEN appt.appointment_id END) / NULLIF(COUNT(appt.appointment_id), 0) COMMENT = 'DNA records divided by all retained appointment records (0-1). Retained records are attended or DNA only, so the denominator is attended + DNA with no cancellations, as in national GP appointment statistics.',
