...
| `sk_patient_id` | `h.sk_patient_id` | Shared patient key when all non-null keys in the action history agree. Null only when every action key is null or non-null keys conflict. Null action keys do not prevent agreement among the populated keys; the referral remains available. |
