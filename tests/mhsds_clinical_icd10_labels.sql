with checked as (
    ...
    inner join {{ ref('icd10_code') }} as d
        on replace({{ clean_icd10_code('upper(trim(c.clinical_code))') }}, '.', '') = d.code
    ...
    inner join {{ ref('icd10_code') }} as d
        on replace({{ clean_icd10_code('upper(trim(a.finding_code))') }}, '.', '') = d.code
